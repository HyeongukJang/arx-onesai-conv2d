...
#if 0
  ervp_task_wait_fx_t task_wait_fx = dca_matrix_conv2d_oneblock_sharedoutput(mop_mapping, i_dca_matrix_conv00_info, num_input, input_info_list, kernel_info_list, output_info, conv_options, init_ouptut);
  flush_cache();
  return task_wait_fx;
#else
...
ervp_task_wait_fx_t i_blocked_matrix_conv_sharedoutput(ervp_mop_mapping_t *mop_mapping, int num_input, const ErvpMatrixInfo **input_info_list, const ErvpMatrixInfo **kernel_info_list, ErvpMatrixInfo *output_info, int conv_options, int init_ouptut)
{
  ...
  blocked_info_conv->block_size = i_dca_matrix_conv00_info->output_matrix_size + kernel_info_list[0]->num_row - 1;
#if 1
  ervp_task_wait_fx_t task_wait_fx = blocked_matrix_conv_sharedoutput(blocked_info_conv, num_input, input_info_list, kernel_info_list, output_info, conv_options, init_ouptut);
  flush_cache();
  return task_wait_fx;
#else
